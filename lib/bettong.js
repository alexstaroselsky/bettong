...
class Bettong {
   ...
   * @param {string} [options.excludePattern="*"] Glob (minimatch) expression to exclude pages from crawling 
   ...
   * - Does href not match (minimatch) exclude pattern?
   * 
   * @param {string} href 
   */
  _isValidPage(href) {
    return href.includes(this.baseUrl) && 
      !_.includes(this.visited, href) && 
      !_.includes(this.queue, href) &&
      !minimatch(href, this.options.excludePattern);
  }
